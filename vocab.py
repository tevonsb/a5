...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        toReturn = []
        for sent in sents:
            ids = [[self.start_of_word] + [self.char2id[x] for x in list(word)] + [self.end_of_word] for word in sent]
            toReturn.append(ids)
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        indexed_sents = self.words2charindices(sents)
        padded_sents = pad_sents_char(indexed_sents, self.char2id['<pad>'])
        ten = torch.tensor(padded_sents, dtype=torch.long, device=device)
        ten = ten.permute(1, 0, 2)
