...
def question_1h_sanity_check():
    ...
    sentence_length = 10
    max_word_length = 21
    highway = Highway(max_word_length, 0.2)
    input = torch.zeros(BATCH_SIZE, max_word_length, dtype=torch.long)
    output = highway.forward(input)
    output_expected_size = [BATCH_SIZE, max_word_length]
...
def question_1i_sanity_check():
    print("-"*80)
    print("Running sanity check for Question 1i (Highway)")
    print("-"*80)
